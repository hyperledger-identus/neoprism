...
- infer the backend from the `NPRISM_DB_URL` scheme and provide a sensible default SQLite path for embedded mode
...
1. Infer the backend from `db_url` (scheme-based) instead of exposing a dedicated CLI flag/env var.
2. Derive the SQLite default URL when `db_url` is omitted (per-platform app-data path that includes the network name) but still allow overriding via `--db-url`.
